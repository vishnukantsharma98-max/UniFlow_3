    ...
    <section className="relative py-14 sm:py-18 lg:py-24 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10 scroll-reveal">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs font-semibold uppercase tracking-wider mb-3 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 rounded-2xl bg-white/[0.06] hover:bg-white/[0.1] border border-purple-500/30 hover:border-purple-400 text-xs font-bold text-purple-300 transition-all flex items-center gap-2 shadow-sm cursor-pointer"
            >
            ...
            <div className="hidden sm:flex items-center gap-1.5">
              <button
                ...
                className="p-2.5 rounded-full bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-slate-300 hover:text-white transition-colors shadow-sm cursor-pointer"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5">
          {reviews.map((rev, idx) => (
            <div
              key={rev.id}
              className={`relative p-5 sm:p-6 rounded-3xl bg-[#0d0722]/90 border border-purple-500/20 backdrop-blur-xl hover:border-purple-400/60 hover:bg-[#150c33] transition-all duration-300 flex flex-col justify-between shadow-[0_8px_25px_rgba(0,0,0,0.5)] hover:shadow-[0_15px_35px_rgba(168,85,247,0.25)] group hover:-translate-y-1 scroll-reveal stagger-${Math.min(idx + 1, 6)}`}
            >
              {/* Quote icon */}
              <div className="flex items-center justify-between mb-4">
                <Quote className="w-7 h-7 text-purple-400/60 group-hover:text-purple-300 transition-colors" />
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-white/10">
                <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-purple-500/40 shadow-sm shrink-0 bg-purple-950">
                  {rev.avatarUrl ? (
                    <img 
                      src={rev.avatarUrl} 
                      alt={rev.name}
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{rev.college} · <span className="text-purple-300 font-semibold">{rev.year}</span></p>
                </div>
              </div>
            </div>
          ))}
