    ...
    <section id="features" className="relative py-14 sm:py-18 lg:py-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-0 right-0 -translate-y-1/2 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-amber-500 opacity-40 z-0 blur-[0.5px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3.5 sm:gap-4 relative z-10">
            {FLOW_STAGES.map((stage, idx) => (
              <div
                key={stage.step}
                className={`group relative p-4 sm:p-5 rounded-3xl bg-[#0d0722]/90 border border-purple-500/20 backdrop-blur-xl hover:border-purple-400/60 hover:bg-[#150c33] transition-all duration-300 flex flex-col justify-between shadow-[0_8px_25px_rgba(0,0,0,0.5)] hover:shadow-[0_15px_35px_rgba(168,85,247,0.25)] hover:-translate-y-1.5 scroll-reveal stagger-${Math.min(idx + 1, 6)}`}
              >
                {/* Step Marker */}
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-mono font-bold text-slate-400 group-hover:text-purple-300 transition-colors">
                    {stage.step}
                  </span>
                  <div 
                    className="p-2.5 rounded-2xl bg-white/[0.06] border border-white/10 group-hover:scale-110 transition-transform shadow-sm"
                ...
                <div className="pt-3.5 mt-3.5 border-t border-white/10 flex items-center justify-between text-[11px] text-purple-300 font-semibold">
                  <span>Flow Link</span>
                  <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-purple-300 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>
            ))}
          </div>
