...
import { StudentAppItem } from './StudentAppsData';
import { useTheme } from '../ThemeSystem';

interface StudentAppCardProps {
  app: StudentAppItem;
  onOpenApp: (app: StudentAppItem) => void;
}

export const StudentAppCard: React.FC<StudentAppCardProps> = ({
  app,
  onOpenApp,
}) => {
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className={`w-13 h-13 rounded-2xl bg-gradient-to-br ${app.gradient} flex items-center justify-center text-white shadow-md group-hover:scale-105 transition-transform shrink-0`}>
      ...
      <div className="mt-6 pt-4 border-t border-black/5 dark:border-white/10 space-y-3">
        {/* Password Info Pill or Direct Access */}
        {app.isDirectAccess ? (
          <div className="flex items-center justify-between text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20">
        ...
        <button
          ...
          className={`w-full py-3 px-4 rounded-xl text-white font-extrabold text-xs sm:text-sm shadow-md hover:shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer ${
            app.isDirectAccess
              ? 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 shadow-emerald-500/20'
              : 'bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 shadow-indigo-500/20'
          }`}
        >
          <span>{app.isDirectAccess ? 'Open Directly' : 'Launch App'}</span>
